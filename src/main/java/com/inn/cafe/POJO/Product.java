...
@NamedQuery(name = "Product.getProductByCategory", query = "select new com.inn.cafe.wrapper.ProductWrapper(p.id,p.name,p.description,p.price,p.status,p.category.id,p.category.name) from Product p where p.category.id =:id and p.status = 'true'")
...
public class Product implements Serializable {
    ...
    public Product(String name, Category category, String description, Integer price, String status, String imageProduct){
        this.name = name;
